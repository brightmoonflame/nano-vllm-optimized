"""INT8 W8A16 vs BF16 generation accuracy: greedy token-match rate.

Runs the same prompts through load-time INT8 W8A16 and BF16 and reports how
faithfully the quantized Linear path reproduces the BF16 greedy output.
KV quantization stays off, so this isolates weight-only PTQ accuracy.
...
def generate(model: str, weight_quant: str | None, prompts: list[str], max_tokens: int) -> list[list[int]]:
    llm = LLM(
        model,
...
def main():
    p = argparse.ArgumentParser(description="INT8 W8A16 vs BF16 greedy generation accuracy.")
    p.add_argument("--model", required=True)
    p.add_argument("--max-tokens", type=int, default=128)
    args = p.parse_args()
    model = str(Path(args.model).expanduser().resolve())

    bf16 = generate(model, weight_quant=None, prompts=PROMPTS, max_tokens=args.max_tokens)
    int8 = generate(model, weight_quant="int8_w8a16", prompts=PROMPTS, max_tokens=args.max_tokens)
