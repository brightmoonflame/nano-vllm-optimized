...
KV configuration, then asserts that the graph model actually replayed both
its Dense Prefill and pure Decode graphs.
"""
...
def run_cases(args: argparse.Namespace, graph_enabled: bool) -> tuple[dict, dict, tuple[int, int]]:
    llm = LLM(
        args.model,
    ...
    try:
        ...
        if graph_enabled and not hasattr(llm.model_runner, "prefill_graphs"):
...
def main() -> None:
    ...
    eager_prefill, eager_decode, eager_replays = run_cases(args, graph_enabled=False)
